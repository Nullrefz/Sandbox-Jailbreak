    ...
    for k, v in pairs(self.logs) do
        ...
        if #boxes > 0 then
            for i = 1, #boxes do
                ...
                if boxes[i]:GetWide() / 2 + x > pos - 64 then
                    offset = offset + 64 + 4
...
function LOGINSPECTOR:DrawPlayerTimeline(x, y, width, height)
    ...
    for k, v in pairs(boxes) do
        if IsValid(v) then
            ...
            self:DrawArrow(posX + v:GetWide() / 2 - 32 - 4, y, 64 + 8, self.timeHeight, 10, Color(20, 20, 20, 255))
            self:DrawArrow(posX + v:GetWide() / 2 - 32, y, 64, self.timeHeight, 10, JB:GetLogColor(v.log.Type))
            draw.DrawRect(posX + v:GetWide() / 2, y, 1, y + 2 + posY - v:GetTall() / 2, JB:GetLogColor(v.log.Type))
...
function LOGINSPECTOR:DrawArrow(x, y, width, height, skew, color)
    ...
    draw.DrawSkewedRect(x, y + height / 2, width, height / 2, skew, Color(200, 200, 200, 30))
end
