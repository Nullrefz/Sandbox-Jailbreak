...
function JB:DrawQuestionPoint(trace)
    
    local wid = 2
    
    cam.Start3D2D(tr.HitPos, Angle(0, 90, 0), math.Clamp(LocalPlayer():GetPos():Distance(tr.HitPos) / 100, 2, 15))
    ...
    cam.End3D2D()
    cam.Start3D2D(tr.HitPos, tr.HitNormal:Angle() + Angle(0, (LocalPlayer():GetEyeTrace().HitPos - LocalPlayer():GetEyeTrace().StartPos):Angle().y, 0), math.Clamp(LocalPlayer():GetPos():Distance(tr.HitPos) / 100, 2, 15))
    local dist = math.Clamp(LocalPlayer():GetPos():Distance(tr.HitPos) / 100 - 1, 0, 1)
    
    if LocalPlayer():GetPos():Distance(tr.HitPos) < 30 then
