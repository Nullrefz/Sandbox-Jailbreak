...
function JB:DrawWaypoint(trace, prog, placed, color, mat)
    ...
    for i = 1, 10 * prog do
       draw.DrawArc(0, 0, 8 * prog, 7 * prog, (1 - math.Clamp(distance, 0, 1000) / 1000) * 33, 36 * i + CurTime() * 25, pointColor)
    end
    ...
    cam.End3D2D()
    cam.Start3D2D(trace.HitPos, trace.HitNormal:Angle() + Angle(0, (LocalPlayer():GetEyeTrace().HitPos - LocalPlayer():GetEyeTrace().StartPos):Angle().y, 0) + Angle(90, -90, 90), math.Clamp(distance / 100, 2, 15))
    local dist = math.Clamp(distance / 100 - 1, 0, 1)
    ...
    if mat then
        draw.DrawRect(-wid , -12 + math.sin(CurTime() * 2) * 2, wid * 2, wid * 2, Color(255, 255, 255, 222 * dist * prog), mat)

    else
        draw.DrawRect(-wid, -8 + math.sin(CurTime() * 2) * 2, wid * 2, wid * 2, Color(255, 255, 255, 222 * dist * prog), mats.ARROW)
    end
